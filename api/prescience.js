      ...
      const result = await cached('prescience_news', CACHE_TTL, async () => {
        ...
        for (const market of activeMarkets) {
          try {
            ...
            // Detect fresh wallets and large positions
            ...
            // Determine price movement signal
            ...
            if (freshWalletCount >= 3 && largePositionCount >= 2) {
              ...
              severity = 'high';
              signal = freshWalletCount >= 2
                ? `${freshWalletCount} fresh wallets entered positions`
                : `${largePositionCount} large positions, ${Math.round(Math.abs(flowImbalance) * 100)}% flow imbalance`;
            } else if (vol24h > 500000 || Math.abs(flowImbalance) > 0.3) {
              severity = 'medium';
              signal = vol24h > 500000
                ? `$${(vol24h / 1e6).toFixed(1)}M 24h volume`
                : `${Math.round(Math.abs(flowImbalance) * 100)}% ${flowImbalance > 0 ? 'buy' : 'sell'} flow imbalance`;
            } else {
            ...
            const headline = dominantOutcome
              ? `"${dominantOutcome[0]}" ${dirStr} to ${pctStr} — ${volStr} new volume`
              : `${volStr} volume surge on active market`;
            ...
            newsItems.push({
              ...
              severity,
              timestamp: new Date().toISOString(),
              flowDirection: flowImbalance > 0.1 ? 'BUY' : flowImbalance < -0.1 ? 'SELL' : 'NEUTRAL',
              freshWallets: freshWalletCount,
              largePositions: largePositionCount,
            });
          } catch {}
        }

        // Sort: critical > high > medium > low, then by volume
