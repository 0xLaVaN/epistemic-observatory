...
const API_BASE = 'https://www.prescience.markets';
...
function EmailSignupForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setStatus('loading');
    try {
      const response = await fetch(`${API_BASE}/prescience/interest`, {
        method: 'POST',
  ...
  const fetchData = useCallback(async () => {
    try {
      const [pulseRes, scanRes] = await Promise.all([
        ...
        fetch(`${API_BASE}/prescience/scanner?limit=3`).then(r => r.ok ? r.json() : null).catch(() => null),
      ]);
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-6">
              <a href="/prescience/scanner" className="text-sm text-white/40 hover:text-white/70 transition-colors">Scanner</a>
              <a href="/prescience/signals" className="text-sm text-white/40 hover:text-white/70 transition-colors">API</a>
              <a href="https://github.com/0xLaVaN/epistemic-observatory" target="_blank" rel="noopener noreferrer" className="text-sm text-white/40 hover:text-white/70 transition-colors">GitHub</a>
            </div>
          </div>
        </div>
      </nav>
      ...
      <main className="pt-24">
        ...
        <section className="relative py-20 px-6">
          <div className="max-w-7xl mx-auto text-center">
            ...
            >
              <button className="px-8 py-4 bg-[#ff3366] text-white font-black tracking-wider rounded-xl hover:bg-[#ff3366]/80 transition-all">
                START MONITORING
              </button>
              <button className="px-8 py-4 bg-white/5 border border-white/20 text-white/80 font-bold tracking-wider rounded-xl hover:bg-white/10 transition-all">
                VIEW API DOCS
              </button>
            </motion.div>
          </div>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            {!loading && demoMarkets.length > 0 && (
              <div className="text-center mt-8">
                <a href="/prescience/scanner" className="inline-flex items-center gap-2 text-[#ff3366] hover:text-[#ff3366]/80 font-bold text-sm transition-colors">
                  VIEW ALL MARKETS →
                </a>
              </div>
            )}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row items-center justify-between">
            ...
            <div className="flex items-center gap-6 text-xs text-white/30">
              <a href="/prescience/scanner" className="hover:text-white/50 transition-colors">Scanner</a>
              <a href="/prescience/signals" className="hover:text-white/50 transition-colors">API</a>
              <a href="https://github.com/0xLaVaN/epistemic-observatory" target="_blank" rel="noopener noreferrer" className="hover:text-white/50 transition-colors">GitHub</a>
